...
async def build_task_card(
    *,
    ...
    is_personal = str(row.get("kind") or "task").lower() == "personal"
    details = ["Личное"] if is_personal else [str(row.get("project_code") or "")]
...
async def _render_task_relations_menu(msg: Message, db_pool: asyncpg.Pool, task_id: int, deps: AppDeps) -> None:
    ...
    async with db_pool.acquire() as conn:
        row = await conn.fetchrow(
            """
            SELECT t.id, t.title, t.status, t.kind, t.deadline, t.project_id, t.parent_task_id,
    ...
    lines = [f"🧩 <b>Связи</b> • #{int(row['id'])}"]
    if is_personal:
        lines.append("Личное")
    else:
        lines.append(f"Проект: <b>{h(str(row.get('project_code') or ''))}</b>")
    if row.get("parent_task_id"):
        lines.append(f"Родитель: #{int(row['parent_task_id'])}")
    lines.append("")
    ...
    if not is_personal:
        kb_rows.append([InlineKeyboardButton(text="📁 В проект…", callback_data=f"task:{task_id}:move")])
    if row.get("parent_task_id"):
        kb_rows.append([
